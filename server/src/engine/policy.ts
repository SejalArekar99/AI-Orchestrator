...
export interface PolicyEvaluationResult {
  ...
  approvalRole?: string;
  policyUsed?: PolicyRule;
  reason: string;
  autoApproved: boolean;
}

export class BusinessPolicyEngine {
  public static evaluate(workflowType: string, metadata: Record<string, any>): PolicyEvaluationResult {
    switch (workflowType) {
      case 'Invoice & Payment Recovery': {
        const amount = metadata.amount || 240000;
        const daysOverdue = metadata.daysOverdue || 67;
        const pol = db.policies.find(p => p.id === 'pol-1');
      ...
      case 'Sales Discount Approval': {
        ...
        const pol = db.policies.find(p => p.id === 'pol-2');
        if (discountPct > 25) {
          return {
            requiresApproval: true,
      ...
      case 'Purchase Request': {
        ...
        const pol = db.policies.find(p => p.id === 'pol-3');
        if (totalCost > 25000) {
          return {
            requiresApproval: true,
